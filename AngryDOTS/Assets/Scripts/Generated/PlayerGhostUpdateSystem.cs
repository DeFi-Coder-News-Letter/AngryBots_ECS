...
public class PlayerLightGhostUpdateSystem : JobComponentSystem
{
    [BurstCompile]
...
#endif
        [ReadOnly] public ArchetypeChunkBufferType<PlayerLightSnapshotData> ghostSnapshotDataType;
...
#endif
                PlayerLightSnapshotData snapshotData;
...
#pragma warning restore 649
    ...
    protected override void OnCreate()
    {
        ...
        {
            All = new []{
                ComponentType.ReadWrite<PlayerLightSnapshotData>(),
        ...
        {
            All = new []{
                ComponentType.ReadOnly<PlayerLightSnapshotData>(),
                ComponentType.ReadOnly<GhostComponent>(),
        ...
        RequireForUpdate(GetEntityQuery(ComponentType.ReadWrite<PlayerLightSnapshotData>(),
            ComponentType.ReadOnly<GhostComponent>()));
    }
    protected override JobHandle OnUpdate(JobHandle inputDeps)
    {
        if (!m_predictedQuery.IsEmptyIgnoreFilter)
        {
            var updatePredictedJob = new UpdatePredictedJob
            {
                ...
                ghostSnapshotDataType = GetArchetypeChunkBufferType<PlayerLightSnapshotData>(true),
...
public partial class PlayerLightGhostSpawnSystem : DefaultGhostSpawnSystem<PlayerLightSnapshotData>
{
    struct SetPredictedDefault : IJobParallelFor
    {
        [ReadOnly] public NativeArray<PlayerLightSnapshotData> snapshots;
        ...
        {
            if (localPlayerId.Length == 1 && snapshots[index].GetPlayerComponentplayerId() == localPlayerId[0].Value)
                predictionMask[index] = 1;
        }
    }
    protected override JobHandle SetPredictedGhostDefaults(NativeArray<PlayerLightSnapshotData> snapshots, NativeArray<int> predictionMask, JobHandle inputDeps)
    {
